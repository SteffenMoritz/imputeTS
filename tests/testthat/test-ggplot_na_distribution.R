...
test_that("Plot with x_axis_labels works and tsNH4 data works", {
  skip_on_cran()
  if (!requireNamespace("ggplot2", quietly = TRUE)) {
    warning("Pkg ggplot2 needed for this test.",
      call. = FALSE
    ...
    # Use zoo to change ts time information to yearmon vector
    # Afterwards create Date vector and from this Date vector POSIXct
    tsNH4_yearmon <- zoo::as.yearmon(time(tsNH4))
    tsNH4_date <- zoo::as.Date(tsNH4_yearmon)
    tsNH4_posix <- as.POSIXct(tsNH4_yearmon)
    ...
    expect_is(
      ggplot_na_distribution(tsNH4, x_axis_labels = tsNH4_date),
      "ggplot"
    )


    expect_is(
      ggplot_na_distribution(tsNH4, x_axis_labels = tsNH4_posix),
      "ggplot"
    )
