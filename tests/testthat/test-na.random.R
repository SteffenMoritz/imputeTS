...
test_that("All NA vector throws error",
          {
            expect_error(na.random(c(NA,NA,NA,NA,NA)))
          })

test_that("Wrong input",
          {
            x <- data.frame(tsAirgap)
            expect_that(na.random(x, lower_bound = 1, upper_bound = -1), throws_error())
            
            x <- rep("string",144)
            x[3] <- NA
            expect_that(na.random(x), throws_error())
...
test_that("Error for lower_bound > upper_bound",
          {
            expect_that( na.random(tsAirgap, lower_bound = 300, upper_bound = 100 ), throws_error())
          })
...
test_that("Test NA at beginning",
          {
            x <- tsAirgap
            x[1:2] <- NA
            expect_that(anyNA(na.random(x, lower_bound = 100, upper_bound = 200 )), is_false())
            expect_that(anyNA(na.random(x, lower_bound = -10, upper_bound = 5000 )), is_false())
            expect_that(anyNA(na.random(x)), is_false())
          })
