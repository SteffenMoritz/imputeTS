context("Depreciated Functions: na.kalman")
...
test_that("Imputation works for data.frame",
          {
            ...
            expect_false(anyNA(na.kalman(x, model="auto.arima", smooth = T)))
            expect_false(anyNA(na.kalman(x, model="auto.arima", smooth = F)))
           })

test_that("Error for wrong input for model parameter",
          {
            expect_error( na.kalman(tsAirgap, model="wrongModel"))
          })

test_that("Error for wrong input for smooth parameter",
          {
            expect_error( na.kalman(tsAirgap, smooth="Wrong"))
          })
...
test_that("Test NA at beginning",
          {
            ...
            expect_false(anyNA(na.kalman(x, model="auto.arima", smooth = T)))
            expect_false(anyNA(na.kalman(x, model="auto.arima", smooth = F)))
            expect_false(anyNA(na.kalman(x, model="StructTS", smooth = T)))
            expect_false(anyNA(na.kalman(x, model="StructTS", smooth = F)))
            expect_false(anyNA(na.kalman(x)), is_false())
...
test_that("Test NA at end",
          {
            ...
            expect_false(anyNA(na.kalman(x, model="StructTS", smooth = T)))
            expect_false(anyNA(na.kalman(x, model="StructTS", smooth = F)))
            expect_false(anyNA(na.kalman(x)))
